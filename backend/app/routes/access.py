...
def create_access_log(
    ...
    if access_log.plate_number:
        vehicle = db.query(UserData).filter(UserData.plate_number == access_log.plate_number).first()
    ...
    new_log = AccessLog(
        ...
        plate_number=access_log.plate_number if vehicle else None,
        entry_time=datetime.utcnow(),
        status=access_log.status
    )
    db.add(new_log)
